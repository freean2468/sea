...
CREATE TABLE sea.sea_user_energy(
	...
	INDEX idx_user_energy_1(sender_id, receiver_id),
	INDEX idx_user_energy_2(sended_time)
);
...
CREATE PROCEDURE sea_LoadEnergyBySender(IN p_sender_id INT) 
	BEGIN
		...
		IF count > 0 THEN
			SELECT receiver_id, sended_time FROM sea_user_energy WHERE sender_id = p_sender_id;
		else
...
CREATE PROCEDURE sea_LoadEnergyByReceiver(IN p_receiver_id INT) 
	BEGIN
		...
		IF count > 0 THEN
			SELECT sender_id, sended_time  FROM sea_user_energy WHERE receiver_id = p_receiver_id;
		ELSE
...
CREATE PROCEDURE sea_AcceptEnergy(IN p_sender_id INT, IN p_receiver_id INT, IN p_sended_time BIGINT)
	BEGIN
		...
		IF count > 0 THEN
			DELETE FROM sea_user_energy WHERE p_sender_id = sender_id AND p_receiver_id = receiver_id AND p_sended_time = sended_time;
...
CREATE PROCEDURE sea_DeleteExpiredEnergy()
	BEGIN
		DELETE FROM sea_user_energy WHERE SUBDATE(NOW(), INTERVAL 20 DAY) > FROM_UNIXTIME(sended_time);
	END
$$
