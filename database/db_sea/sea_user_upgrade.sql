...
CREATE PROCEDURE sea_LoadUpgradeInfo(IN p_id INT)
	BEGIN
		SELECT honey_score, honey_time, cool_down FROM sea_user_upgrade WHERE id = p_id;
	END
$$
...
CREATE PROCEDURE sea_UpgradeCoolDown(IN p_id INT)
	BEGIN
		UPDATE sea_user_upgrade SET cool_down = cool_down + 1 WHERE id = p_id;
	END
$$
