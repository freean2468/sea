...
CREATE PROCEDURE sea_AddCharacterTwo(IN p_id INT)
	BEGIN
		DECLARE res TINYINT UNSIGNED;
		UPDATE sea_user_characters SET res = character_two + 1, character_two = character_two + 1 WHERE id = p_id;
		SELECT res AS res;
	END
$$
...
CREATE PROCEDURE sea_AddCharacterThree(IN p_id INT)
	BEGIN
		DECLARE res TINYINT UNSIGNED;
		UPDATE sea_user_characters SET res = character_three + 1, character_three = character_three + 1 WHERE id = p_id;
		SELECT res AS res;
	END
$$
...
CREATE PROCEDURE sea_AddCharacterFour(IN p_id INT)
	BEGIN
		DECLARE res TINYINT UNSIGNED;
		UPDATE sea_user_characters SET res = character_four + 1, character_four = character_four + 1 WHERE id = p_id;
		SELECT res AS res;
	END
$$
