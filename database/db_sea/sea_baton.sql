...
DROP TABLE IF EXISTS sea_baton;

CREATE TABLE sea.sea_baton (
	...
	INDEX idx_baton_1(sender_id, receiver_id),
	INDEX idx_baton_2(sended_time)
);
...
CREATE PROCEDURE sea_AddBaton(IN p_sender_id INT, IN p_receiver_id INT, IN p_score INT UNSIGNED, IN p_map VARCHAR(10) CHARACTER SET utf8)
	BEGIN
		INSERT sea_baton(sender_id, receiver_id, score, map, sended_time)
...
CREATE PROCEDURE sea_ExistBaton(IN p_sender_id INT, IN p_receiver_id INT, IN p_sended_time BIGINT UNSIGNED) 
	BEGIN
		...
		SELECT COUNT(*) INTO count FROM sea_baton WHERE receiver_id = p_receiver_id AND sender_id = p_sender_id AND p_sended_time = sended_time;
...
CREATE PROCEDURE sea_LoadBatonScore(IN p_sender_id INT, IN p_receiver_id INT, IN p_sended_time BIGINT UNSIGNED) 
	BEGIN
		...
		IF count > 0 THEN
			SELECT score FROM sea_baton WHERE sender_id = p_sender_id AND receiver_id = p_receiver_id AND sended_time = p_sended_time;
		ELSE
			SELECT -1 AS score;
		END IF;
	END
...
CREATE PROCEDURE sea_LoadBaton(IN p_receiver_id INT) 
	BEGIN
		...
		SELECT COUNT(*) INTO count FROM sea_baton WHERE receiver_id = p_receiver_id;

		IF count > 0 THEN
			SELECT sender_id, score, map, sended_time FROM sea_baton WHERE receiver_id = p_receiver_id;
		ELSE
...
CREATE PROCEDURE sea_DeleteBaton(IN p_sender_id INT, IN p_receiver_id INT, IN p_sended_time BIGINT UNSIGNED)
	BEGIN
		DELETE FROM sea_baton WHERE sender_id = p_sender_id AND receiver_id = p_receiver_id AND sended_time = p_sended_time;
	END
$$
...
CREATE PROCEDURE sea_DeleteExpiredBaton()
	BEGIN
		DELETE FROM sea_baton WHERE SUBDATE(NOW(), INTERVAL 20 DAY) > FROM_UNIXTIME(sended_time);
	END
$$
